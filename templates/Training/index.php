         ...
         <div class="row">
            <div class="col-lg-6 col-md-12 col-sm-12">
               <div class="heading ft-secondary">
                    <span class="icon"><i class="fa fa-user"></i></span>Training
                    <nav>
                        <ol class="breadcrumb">
            ...
            <div class="col-lg-2 col-sm-4 align-bottom pdr-0">
               <div class="actions-ctrl text-md-right mt-2">
                <?= $this->Html->link('<i class="fa fa-list"></i><span>Courses</span>', [
            ...
            <div class="col-lg-2 col-sm-4 align-bottom pdr-0">
               <div class="actions-ctrl text-md-right mt-2">
                <?= $this->Html->link('<i class="fa fa-list"></i><span>Lessions</span>', [
                     "controller" => "Lessions", "action" => "index"
                  ], [
                     'class' => "v-btn v-btn-secondary btn-block",
                     "escape" => false,
                  ]); ?>              
            ...
            <div class="col-lg-2 col-sm-4 align-bottom pdr-0">
               <div class="actions-ctrl text-md-right mt-2">
                <?= $this->Html->link('<i class="fa fa-plus"></i><span>Add Training</span>', [
                  ], [
                     'class' => "v-btn v-btn-secondary btn-block",
                     "escape" => false,
        ...
        <div class="row">
                ...
                <?= $this->Flash->render() ?>
                    <table  class="table table-light nowrap table-sm"  id="example" style="width:100%">
                        ...
                        <tbody>
                            <?php $i=1; 
                            foreach ($training as $tValue): ?>
                            <tr>
                                <td><?= $i; ?></td>
                                <td><?= $tValue->name ?></td>
                                <td>
                                    <input class="tgl tgl-light change-status" id="<?= $tValue->id;?>" type="checkbox" value="<?=$tValue->status; ?>"  <?= $tValue->status == 1?'checked':'' ?>/>
                                    <label class="tgl-btn" for="<?= $tValue->id;?>"></label>
                                </td>
                                <td>
                                    <!-- <?= $this->Html->link('<i class="fa fa-pencil-alt"></i>','/CareerTracks/edit/'.$careerTrack->id,['class' => 'icon ft-primary icon-sm','escape' => false]); ?> -->
                                    <a href="#" class="" onClick="editTraining(<?=$tValue->id?>)" >
                                        <i class="fa fa-pencil-alt"></i><span></span>
                                    </a>

                                    <a href="" data-toggle="modal" data-id="<?= $tValue->id;?>" data-target="#confirm" data-type="entry" class="icon icon-sm delete" data-url="<?= $this->Url->build()?>">
                                      <i class="fa fa-archive"></i>
                                    </a> 
                                                                         
                                    <!-- <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $tValue->id], ['confirm' => __('Are you sure you want to delete # {0}?', $careerTrack->id)]) ?> -->
                                </td>
                            </tr>
                            <?php $i++; endforeach; ?>
                        </tbody>
...
<!-- add Training -->
<div class="modal fade" tabindex="-1" role="dialog" id="add_training">
...
<!-- edit training -->
<div class="modal fade" tabindex="-1" role="dialog" id="edit_training" data-backdrop="static" data-keyboard="false">
   ...
   //Status Change
       $('.change-status').click(function() {
         ...
         var status = $(this).val();               
               ...
               $.ajax({                  
                   type:'GET',
                   url:"<?= $this->Url->build('/Training/updateStatus/'); ?>"+id+'/'+status,                  
                   beforeSend: function (){
                    $.LoadingOverlay("show");
                   },
    ...
    function editTraining(id)
        {
            $.ajax({
                url:  "<?= $this->Url->build('/Training/edit/'); ?>"+id,
                type: 'GET',
                beforeSend: function() { 
                    // $.LoadingOverlay("show");
                },
